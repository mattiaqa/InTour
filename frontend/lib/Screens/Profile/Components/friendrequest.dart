...
class FriendRequestButtonState extends State<FirendRequestButton> {
                    ...
                    children: [
                      SizedBox(
                        ...
                        child: ElevatedButton(
                            onPressed: () {
                              ApiManager.postData('friends/accept',
                                  {'username': widget.username}).then((value) {
                                widget.onTap();
                                ShowBottomMessage(
                                    context, 'Hai un nuovo amico!');
                              });
                            },
                      ...
                      SizedBox(
                        ...
                        child: ElevatedButton(
                            onPressed: () {
                              ApiManager.postData('friends/reject',
                                  {'username': widget.username}).then((value) {
                                widget.onTap();
                                ShowBottomMessage(
                                    context, 'Richiesta rifiutata');
                              });
                            },
                            child: Text('Rifiuta')),
                      ),
                    ...
                    onPressed: () {
                      ...
                      if (status == FriendshipState.friends) {
                        ApiManager.postData('friends/remove', frienddata)
                          ...
                          widget.onTap();
                          ShowBottomMessage(
                              context, 'La vostra amicizia termina qui');
                        });
  ...
  FriendshipState friendStatus() {
    ...
    if ((widget.username == currentUser)) return FriendshipState.sameUser;

    if (widget.friends.contains(currentUser)) return FriendshipState.friends;
