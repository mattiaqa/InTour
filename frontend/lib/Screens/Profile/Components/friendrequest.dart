...
class FirendRequestButton extends StatefulWidget {
  String username;
  FirendRequestButton({super.key, required this.username});
...
class FriendRequestButtonState extends State<FirendRequestButton> {
    ...
    return FutureBuilder<FriendshipState>(
        ...
        builder: ((context, snapshot) {
          ...
          if (snapshot.hasError) {
            return Text(
                'Errore durante il recupero dello stato di amicizia: ${snapshot.error}');
          }
                      ...
                      onPressed: () {
                        ...
                        if (snapshot.data == FriendshipState.friends) {
                          ApiManager.postData('friends/remove', frienddata)
                            ...
                            FriendshipState.requestRecieved) {
  ...
  Future<FriendshipState> _friendStatus() async {
    ...
    String? data = await ApiManager.fetchData(
        'profile/${AppService.instance.currentUser!.userid}/data');
    ...
    if (profile['friends'] != null) {
      ...
      if (friends.contains(widget.username)) return FriendshipState.friends;
    }

    if (profile['friends_pending'] != null) {
      List<dynamic> friends_to_wait =
          profile['friends_pending'] as List<dynamic>;
